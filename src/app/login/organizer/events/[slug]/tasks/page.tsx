...
export default function TasksPage() {
  ...
  const inp = (value: string, onChange: (v: string) => void, placeholder: string) => (
    <input
      ...
      style={{ padding: "7px 10px", border: "1px solid #e4ebe6", borderRadius: "7px", fontSize: "0.85rem", fontFamily: "Georgia, serif", width: "100%", boxSizing: "border-box" as const }}
    />
  );
  ...
  const sel = (value: string, onChange: (v: string) => void) => (
    <select
      value={value}
      onChange={e => onChange(e.target.value)}
      style={{ padding: "7px 10px", border: "1px solid #e4ebe6", borderRadius: "7px", fontSize: "0.85rem", fontFamily: "Georgia, serif", background: "#fff" }}
    >
    ...
    <div style={{ minHeight: "100vh", background: "#f8faf8", fontFamily: "Georgia, serif", padding: "2rem 1.5rem" }}>
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          <Link href={`/login/organizer/events/${slug}`} style={{ fontSize: "0.85rem", color: "#4a5a52", textDecoration: "none" }}>← Back to {city}</Link>
          <h1 style={{ fontSize: "1.8rem", color: "#1B3A2D", fontWeight: "normal", marginTop: "0.5rem" }}>Brand To-Do List Manager</h1>
          <p style={{ color: "#4a5a52", fontSize: "0.9rem", marginTop: "4px" }}>Every task here appears in all brand portals automatically. {deadlines.length} tasks total.</p>
        </div>

        <div style={{ background: "#fff", borderRadius: "16px", padding: "1.5rem", border: "1px solid #e4ebe6", marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
            <div style={{ fontSize: "1rem", color: "#1B3A2D" }}>All tasks</div>
          ...
          {adding && (
            <div style={{ background: "#f8faf8", borderRadius: "12px", padding: "1rem", marginBottom: "1.25rem", border: "1px solid #e4ebe6" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button onClick={addDeadline} style={{ padding: "7px 16px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.85rem", cursor: "pointer", fontFamily: "Georgia, serif" }}>Save</button>
            ...
            return (
              <div key={cat} style={{ marginBottom: "1.5rem" }}>
                ...
                {catDeadlines.map(deadline => (
                  <div key={deadline.id} style={{ borderRadius: "10px", border: "1px solid #f0f4f1", background: "#f8faf8", padding: "10px 12px", marginBottom: "6px" }}>
                    {editing === deadline.id ? (
                      <div style={{ display: "flex", flexDirection: "column" as const, gap: "6px" }}>
                        ...
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button onClick={() => saveEdit(deadline.id)} style={{ padding: "5px 12px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "6px", fontSize: "0.8rem", cursor: "pointer" }}>Save</button>
                          <button onClick={() => setEditing(null)} style={{ padding: "5px 12px", background: "transparent", border: "1px solid #e4ebe6", borderRadius: "6px", fontSize: "0.8rem", cursor: "pointer" }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <div style={{ fontSize: "0.88rem", color: "#1B3A2D", fontFamily: "Georgia, serif" }}>{deadline.task}</div>
                          <div style={{ fontSize: "0.75rem", color: "#4a5a52", marginTop: "2px" }}>Due {deadline.due_date}</div>
                        </div>
                        <div style={{ display: "flex", gap: "4px" }}>
                          <button onClick={() => startEdit(deadline)} title="Edit" style={{ background: "transparent", border: "none", cursor: "pointer", padding: "4px 5px", borderRadius: "5px", color: "#E8C97A", display: "flex", alignItems: "center", opacity: 0.7 }} onMouseEnter={e => { e.currentTarget.style.background = "#f0f4f1"; e.currentTarget.style.opacity = "1"; }} onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.opacity = "0.7"; }}>
                            <PencilIcon />
                          </button>
        ...
        <div style={{ background: "#fdf8f3", borderRadius: "12px", padding: "1.25rem", border: "1px solid #e4ebe6" }}>
          <div style={{ fontSize: "0.85rem", color: "#E8C97A", marginBottom: "4px", fontWeight: 500 }}>How this works</div>
          <div style={{ fontSize: "0.82rem", color: "#4a5a52", lineHeight: 1.7 }}>
