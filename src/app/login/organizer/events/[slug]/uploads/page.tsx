      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "12px", marginBottom: "1.5rem" }}>
          <div style={{ background: "#fff", borderRadius: "12px", padding: "1rem 1.25rem", border: "1px solid #e4ebe6" }}>
            <div style={{ fontSize: "0.7rem", color: "#4a5a52", marginBottom: "4px" }}>TOTAL FILES</div>
            <div style={{ fontSize: "1.8rem", color: "#1B3A2D" }}>{files.length}</div>
          </div>
          <div style={{ background: "#fff", borderRadius: "12px", padding: "1rem 1.25rem", border: "1px solid #e4ebe6" }}>
            <div style={{ fontSize: "0.7rem", color: "#4a5a52", marginBottom: "4px" }}>BRANDS UPLOADED</div>
            <div style={{ fontSize: "1.8rem", color: "#1B3A2D" }}>{new Set(files.map(f => f.brand)).size}</div>
          </div>
          <div style={{ background: "#fff", borderRadius: "12px", padding: "1rem 1.25rem", border: "1px solid #e4ebe6" }}>
        ...
        <div style={{ display: "flex", gap: "12px", marginBottom: "1.5rem", flexWrap: "wrap" as const }}>
          ...
          <div>
            <label style={{ fontSize: "0.75rem", color: "#4a5a52", display: "block", marginBottom: "4px" }}>CATEGORY</label>
            <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} style={{ padding: "8px 12px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.85rem", fontFamily: "Georgia, serif", color: "#1B3A2D", background: "#fff" }}>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "3rem", color: "#4a5a52" }}>Loading files...</div>
        ) : filtered.length === 0 ? (
          <div style={{ background: "#fff", borderRadius: "12px", padding: "3rem", textAlign: "center", border: "1px solid #e4ebe6" }}>
            <div style={{ fontSize: "2rem", marginBottom: "1rem" }}>📭</div>
            <div style={{ color: "#4a5a52", fontSize: "0.9rem" }}>No files uploaded yet</div>
          </div>
        ) : (
          <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e4ebe6", overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #e4ebe6", background: "#f8faf8" }}>
                  {["File", "Brand", "Category", "Size", "Uploaded", ""].map((h, i) => (
                    <th key={i} style={{ textAlign: "left", padding: "10px 14px", fontSize: "11px", color: "#4a5a52", fontWeight: 400 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={i} style={{ borderBottom: "1px solid #f0f4f1" }}>
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        ...
                        <span style={{ color: "#1B3A2D", fontWeight: 500 }}>{file.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: "12px 14px", color: "#4a5a52" }}>{file.brand}</td>
                    <td style={{ padding: "12px 14px" }}>
                      <span style={{ fontSize: "11px", padding: "2px 8px", borderRadius: "20px", background: "#E8C97A22", color: "#E8C97A" }}>{file.category}</span>
                    </td>
                    <td style={{ padding: "12px 14px", color: "#4a5a52" }}>{formatSize(file.size)}</td>
                    <td style={{ padding: "12px 14px", color: "#4a5a52" }}>{formatDate(file.uploaded_at)}</td>
                    <td style={{ padding: "12px 14px" }}>
                      <a href={file.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: "11px", padding: "4px 12px", background: "#1B3A2D", color: "#fff", borderRadius: "6px", textDecoration: "none", fontFamily: "Georgia, serif" }}>View</a>
                    </td>
                  </tr>
                ))}
              </tbody>
