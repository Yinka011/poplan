...
export default function PlannerDashboard() {
  ...
  if (loading) return <div style={{ minHeight: "100vh", background: "#faf8f5", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Georgia, serif", color: "#6b5f54" }}>Loading...</div>;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#faf8f5", fontFamily: "Georgia, serif" }}>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: "220px", background: "#5a3e2b", zIndex: 16, transform: sidebarOpen ? "translateX(0)" : "translateX(-100%)", transition: "transform 0.25s ease", display: "flex", flexDirection: "column" as const, paddingTop: "60px" }}>
        <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid #6b4f3a" }}>
          ...
          {plannerEvent?.city && <div style={{ fontSize: "0.75rem", color: "#c4956a", marginTop: "2px" }}>{plannerEvent.city}</div>}
        </div>
        <nav style={{ flex: 1, padding: "1rem 0" }}>
          {tabs.map(tab => (
            <a key={tab.key} onClick={() => { setActiveTab(tab.key as any); setSidebarOpen(false); }} style={{ display: "block", padding: "10px 1.25rem", fontSize: "0.85rem", color: activeTab === tab.key ? "#fff" : "#c8b89a", background: activeTab === tab.key ? "#6b4f3a" : "transparent", textDecoration: "none", borderLeft: activeTab === tab.key ? "2px solid #c4956a" : "2px solid transparent", cursor: "pointer" }}>
              {tab.label}
            </a>
          ))}
        </nav>
      ...
      {/* Top bar */}
      <div style={{ background: "#5a3e2b", padding: "0.85rem 1.5rem", display: "flex", alignItems: "center", gap: "1rem", position: "sticky" as const, top: 0, zIndex: 14 }}>
        <button onClick={() => setSidebarOpen(!sidebarOpen)} style={{ background: "transparent", border: "none", cursor: "pointer", padding: "4px", display: "flex", flexDirection: "column" as const, gap: "5px" }}>
          <span style={{ display: "block", width: "20px", height: "1.5px", background: sidebarOpen ? "#c4956a" : "#c8b89a", transition: "all 0.2s", transform: sidebarOpen ? "rotate(45deg) translate(4.5px, 4.5px)" : "none" }} />
      ...
      <div style={{ padding: "2rem 2.5rem", maxWidth: "1000px", margin: "0 auto" }}>
        ...
        <div style={{ background: "#5a3e2b", borderRadius: "16px", padding: "1.75rem 2rem", marginBottom: "1.5rem", color: "#fff" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "1.5rem", alignItems: "start" }}>
            ...
            <div>
              ...
              <div style={{ height: "3px", background: "#6b4f3a", borderRadius: "2px", marginTop: "8px" }}>
                <div style={{ height: "100%", width: `${progress}%`, background: "#c4956a", borderRadius: "2px" }} />
              </div>
            ...
            <div style={{ background: "#fff", borderRadius: "10px", padding: "1rem", textAlign: "center" as const }}>
              <div style={{ fontSize: "2.5rem", color: "#5a3e2b", fontFamily: "Georgia, serif", fontWeight: "normal", lineHeight: 1 }}>{daysToEvent ?? "—"}</div>
              <div style={{ fontSize: "0.7rem", color: "#6b5f54", marginTop: "6px", letterSpacing: "0.05em" }}>DAYS TO EVENT</div>
            </div>
          </div>
        </div>
        ...
        {activeTab === "overview" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
              <div style={{ fontSize: "0.75rem", color: "#6b5f54", letterSpacing: "0.08em", marginBottom: "1rem" }}>TASK PROGRESS</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                <div>
                  <div style={{ fontSize: "0.72rem", color: "#6b5f54", marginBottom: "4px" }}>MY TASKS</div>
                  <div style={{ fontSize: "1.3rem", color: "#5a3e2b" }}>{myCompleted}/{myTasks.length}</div>
                </div>
                <div>
                  <div style={{ fontSize: "0.72rem", color: "#6b5f54", marginBottom: "4px" }}>{plannerEvent?.brand_name?.toUpperCase() || "BRAND"}</div>
                  <div style={{ fontSize: "1.3rem", color: "#5a3e2b" }}>{brandCompleted}/{brandTasks.length}</div>
                </div>
              </div>
            </div>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
              <div style={{ fontSize: "0.75rem", color: "#6b5f54", letterSpacing: "0.08em", marginBottom: "1rem" }}>BUDGET BREAKDOWN</div>
              {[
                { label: "Decor", value: totalDecor, color: "#c4956a" },
                ...
                <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: i < 3 ? "1px solid #f0ece6" : "none" }}>
                  <span style={{ fontSize: "0.85rem", color: "#6b5f54" }}>{item.label}</span>
                  <span style={{ fontSize: "0.85rem", color: item.color, fontWeight: 500 }}>${item.value.toFixed(2)}</span>
                </div>
              ))}
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px", paddingTop: "8px", borderTop: "2px solid #5a3e2b" }}>
                <span style={{ fontSize: "0.9rem", color: "#5a3e2b", fontWeight: 500 }}>Total</span>
                <span style={{ fontSize: "0.9rem", color: "#5a3e2b", fontWeight: 500 }}>${grandTotal.toFixed(2)}</span>
              </div>
            </div>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
              <div style={{ fontSize: "0.75rem", color: "#6b5f54", letterSpacing: "0.08em", marginBottom: "1rem" }}>RECEIPTS</div>
              <div style={{ fontSize: "1.3rem", color: "#5a3e2b", marginBottom: "4px" }}>{receipts.length}</div>
              <div style={{ fontSize: "0.8rem", color: "#6b5f54" }}>receipt{receipts.length !== 1 ? "s" : ""} uploaded</div>
              <div style={{ fontSize: "0.8rem", color: "#4a7c59", marginTop: "4px" }}>{receipts.filter(r => r.status === "approved").length} approved</div>
            </div>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
              <div style={{ fontSize: "0.75rem", color: "#6b5f54", letterSpacing: "0.08em", marginBottom: "1rem" }}>MESSAGES</div>
              <div style={{ fontSize: "1.3rem", color: "#5a3e2b", marginBottom: "4px" }}>{messages.length}</div>
              <div style={{ fontSize: "0.8rem", color: "#6b5f54" }}>message{messages.length !== 1 ? "s" : ""} exchanged</div>
            </div>
          </div>
        )}
          ...
          <div>
            <div style={{ display: "flex", gap: "8px", marginBottom: "1.5rem" }}>
              {(["decor", "refreshments", "staff"] as const).map(t => (
                <button key={t} onClick={() => setPlanningTab(t)} style={{ padding: "8px 20px", background: planningTab === t ? "#5a3e2b" : "#fff", color: planningTab === t ? "#fff" : "#6b5f54", border: "1px solid " + (planningTab === t ? "#5a3e2b" : "#e8e2da"), borderRadius: "20px", fontSize: "0.85rem", cursor: "pointer", fontFamily: "Georgia, serif", textTransform: "capitalize" as const }}>{t}</button>
              ))}
              <a href={`/login/organizer/planner/${slug}/planning`} style={{ marginLeft: "auto", padding: "8px 16px", background: "#5a3e2b", color: "#fff", borderRadius: "20px", fontSize: "0.85rem", textDecoration: "none", fontFamily: "Georgia, serif" }}>Manage items →</a>
            </div>
            ...
            {planningTab === "decor" && (
              <div>
                {["Furniture", "Props", "Lighting", "Signage", "Theme", "Florals"].map(cat => {
                  ...
                  return (
                    <div key={cat} style={{ marginBottom: "1rem" }}>
                      <div style={{ fontSize: "0.75rem", color: "#6b5f54", letterSpacing: "0.08em", marginBottom: "8px" }}>{cat.toUpperCase()}</div>
                      <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8e2da", overflow: "hidden" }}>
                        {items.map((item, i) => (
                          <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 16px", borderBottom: i < items.length - 1 ? "1px solid #f0ece6" : "none" }}>
                            <div>
                              <div style={{ fontSize: "0.88rem", color: "#5a3e2b" }}>{item.item}</div>
                              {item.notes && <div style={{ fontSize: "0.75rem", color: "#6b5f54" }}>{item.notes}</div>}
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                              {item.quantity > 0 && <span style={{ fontSize: "0.78rem", color: "#6b5f54" }}>Qty: {item.quantity}</span>}
                              <span style={{ fontSize: "0.9rem", color: "#c4956a", fontWeight: 500 }}>${Number(item.cost).toFixed(2)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                {decor.length === 0 && <p style={{ fontSize: "0.85rem", color: "#6b5f54" }}>No decor items yet. Click Edit & Add Items to add.</p>}
              </div>
            )}

            {planningTab === "refreshments" && (
              <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8e2da", overflow: "hidden" }}>
                {refresh.length === 0 && <p style={{ padding: "1rem", fontSize: "0.85rem", color: "#6b5f54" }}>No refreshments yet.</p>}
                {refresh.map((item, i) => (
                  <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 16px", borderBottom: i < refresh.length - 1 ? "1px solid #f0ece6" : "none" }}>
                    <div>
                      <div style={{ fontSize: "0.88rem", color: "#5a3e2b" }}>{item.item}</div>
                      {item.quantity && <div style={{ fontSize: "0.75rem", color: "#6b5f54" }}>{item.quantity}</div>}
                    </div>
            ...
            {planningTab === "staff" && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "10px" }}>
                ...
                {staff.map(member => {
                  ...
                  return (
                    <div key={member.id} style={{ background: "#fff", borderRadius: "12px", padding: "1rem", border: "1px solid #e8e2da" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div>
                          <div style={{ fontSize: "0.95rem", color: "#5a3e2b", marginBottom: "4px" }}>{member.name}</div>
                          {member.notes && <div style={{ fontSize: "0.75rem", color: "#6b5f54" }}>{member.notes}</div>}
                        </div>
                        <div style={{ textAlign: "right" as const }}>
                          {totalHours > 0 && <div style={{ fontSize: "1rem", color: "#c4956a", fontWeight: 500 }}>${totalPay.toFixed(2)}</div>}
                          {totalHours > 0 && <div style={{ fontSize: "0.72rem", color: "#6b5f54" }}>{totalHours}hrs</div>}
                        </div>
                      </div>
                    </div>
                  );
        ...
        {activeTab === "budget" && (
          <div>
            <div style={{ background: "#5a3e2b", borderRadius: "12px", padding: "1.25rem 1.5rem", marginBottom: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {["Venue", "Content", "Marketing", "Operations", "Logistics"].map(cat => {
              ...
              return (
                <div key={cat} style={{ background: "#fff", borderRadius: "12px", padding: "1.25rem", marginBottom: "1rem", border: "1px solid #e8e2da" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
                    <div style={{ fontSize: "0.85rem", color: "#5a3e2b", fontWeight: 500 }}>{cat}</div>
                    {catTotal > 0 && <div style={{ fontSize: "0.95rem", color: "#c4956a", fontWeight: 500 }}>${catTotal.toFixed(2)}</div>}
                  </div>
                  {items.map(exp => (
                    <div key={exp.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #f0ece6", fontSize: "0.85rem", alignItems: "center" }}>
                      <span style={{ color: "#5a3e2b" }}>{exp.item}</span>
                      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                        {exp.deposit > 0 && <span style={{ fontSize: "0.75rem", color: "#4a7c59" }}>Deposit: ${Number(exp.deposit).toFixed(2)}</span>}
                        <span style={{ color: "#c4956a", fontWeight: 500 }}>${Number(exp.cost).toFixed(2)}</span>
                  ...
                  {addingExpense === cat ? (
                    <div style={{ marginTop: "8px", display: "flex", gap: "8px", flexWrap: "wrap" as const, alignItems: "center" }}>
                      <input placeholder="Item description" value={newExpense.item} onChange={e => setNewExpense(prev => ({...prev, item: e.target.value}))} style={{ flex: 1, padding: "6px 8px", border: "1px solid #e8e2da", borderRadius: "6px", fontSize: "0.82rem", fontFamily: "Georgia, serif", minWidth: "150px" }} />
                      <input placeholder="Cost $" value={newExpense.cost} onChange={e => setNewExpense(prev => ({...prev, cost: e.target.value}))} style={{ width: "80px", padding: "6px 8px", border: "1px solid #e8e2da", borderRadius: "6px", fontSize: "0.82rem", fontFamily: "Georgia, serif" }} />
                      <input placeholder="Deposit $" value={newExpense.deposit} onChange={e => setNewExpense(prev => ({...prev, deposit: e.target.value}))} style={{ width: "80px", padding: "6px 8px", border: "1px solid #e8e2da", borderRadius: "6px", fontSize: "0.82rem", fontFamily: "Georgia, serif" }} />
                      <button onClick={() => addExpense(cat)} style={{ padding: "6px 12px", background: "#5a3e2b", color: "#fff", border: "none", borderRadius: "6px", fontSize: "0.82rem", cursor: "pointer" }}>Save</button>
                      <button onClick={() => setAddingExpense(null)} style={{ padding: "6px 12px", background: "transparent", border: "1px solid #e8e2da", borderRadius: "6px", fontSize: "0.82rem", cursor: "pointer" }}>Cancel</button>
                    </div>
                  ) : (
                    <button onClick={() => { setAddingExpense(cat); setNewExpense({ item: "", cost: "", deposit: "" }); }} style={{ marginTop: "8px", fontSize: "0.78rem", padding: "4px 10px", background: "transparent", border: "1px solid #e8e2da", borderRadius: "6px", cursor: "pointer", color: "#6b5f54" }}>+ Add {cat.toLowerCase()} expense</button>
                  )}
                </div>
              );
            })}
            ...
            {[
              ...
              <div key={section.label} style={{ background: "#fff", borderRadius: "12px", padding: "1.25rem", marginBottom: "1rem", border: "1px solid #e8e2da" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: section.color }} />
                    <div style={{ fontSize: "0.85rem", color: "#5a3e2b", fontWeight: 500 }}>{section.label}</div>
                    <span style={{ fontSize: "0.7rem", color: "#6b5f54", background: "#f0ece6", padding: "1px 6px", borderRadius: "10px" }}>Planning Hub</span>
                  </div>
                ...
                {section.items.filter(i => i.cost > 0).map((item, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #f8f5f0", fontSize: "0.82rem" }}>
                    <div>
                      <span style={{ color: "#5a3e2b" }}>{item.name}</span>
                      {item.detail && <span style={{ color: "#6b5f54", fontSize: "0.72rem", marginLeft: "6px" }}>{item.detail}</span>}
                    </div>
        ...
        {activeTab === "mytasks" && (
          <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
            <div style={{ fontSize: "0.9rem", color: "#5a3e2b", marginBottom: "1rem" }}>My Tasks ({myCompleted}/{myTasks.length})</div>
            <div style={{ display: "flex", gap: "8px", marginBottom: "1rem", flexWrap: "wrap" as const }}>
              ...
              <button onClick={addMyTask} style={{ padding: "8px 16px", background: "#5a3e2b", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.85rem", cursor: "pointer" }}>Add</button>
            </div>
            {myTasks.length === 0 && <p style={{ fontSize: "0.85rem", color: "#6b5f54" }}>No tasks yet. Add your first task above.</p>}
            {myTasks.map(task => (
              <div key={task.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px", borderRadius: "8px" }} onMouseEnter={e => (e.currentTarget.style.background = "#faf8f5")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                <div onClick={() => toggleTask(task, true)} style={{ width: "18px", height: "18px", borderRadius: "50%", border: task.completed ? "none" : "2px solid #d4c5b0", background: task.completed ? "#c4956a" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                  {task.completed && <span style={{ color: "#fff", fontSize: "10px" }}>✓</span>}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: "0.88rem", color: task.completed ? "#b0a090" : "#5a3e2b", textDecoration: task.completed ? "line-through" : "none" }}>{task.task}</div>
                  {task.due_date && <div style={{ fontSize: "0.72rem", color: "#6b5f54" }}>Due {task.due_date}</div>}
                </div>
        ...
        {activeTab === "brandtasks" && (
          <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
            <div style={{ fontSize: "0.9rem", color: "#5a3e2b", marginBottom: "0.5rem" }}>{plannerEvent?.brand_name || "Brand"} Tasks ({brandCompleted}/{brandTasks.length})</div>
            <p style={{ fontSize: "0.8rem", color: "#6b5f54", marginBottom: "1rem" }}>These tasks are visible to {plannerEvent?.brand_name || "the brand"} on their portal. Assign to a team member if needed.</p>
            <div style={{ display: "flex", gap: "8px", marginBottom: "1rem", flexWrap: "wrap" as const }}>
              ...
              <button onClick={addBrandTask} style={{ padding: "8px 16px", background: "#5a3e2b", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.85rem", cursor: "pointer" }}>Add</button>
            </div>
            {brandTasks.length === 0 && <p style={{ fontSize: "0.85rem", color: "#6b5f54" }}>No tasks yet.</p>}
            {brandTasks.map(task => (
              <div key={task.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px", borderRadius: "8px" }} onMouseEnter={e => (e.currentTarget.style.background = "#faf8f5")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", marginTop: "2px" }}>
                    {task.due_date && <span style={{ fontSize: "0.72rem", color: "#6b5f54" }}>Due {task.due_date}</span>}
                    {task.assigned_to && <span style={{ fontSize: "0.72rem", color: "#c4956a" }}>→ {task.assigned_to}</span>}
                  </div>
                </div>
        ...
        {activeTab === "chat" && (
          <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
            <div style={{ fontSize: "0.9rem", color: "#5a3e2b", marginBottom: "1rem" }}>Chat with {plannerEvent?.brand_name || "brand"}</div>
            <div style={{ height: "400px", overflowY: "auto", marginBottom: "1rem", display: "flex", flexDirection: "column" as const, gap: "10px", padding: "0.5rem" }}>
              {messages.length === 0 && <p style={{ fontSize: "0.85rem", color: "#6b5f54", textAlign: "center", marginTop: "2rem" }}>No messages yet. Start the conversation.</p>}
              {messages.map(msg => {
                const isMe = msg.sender_email === userEmail;
                return (
                  <div key={msg.id} style={{ display: "flex", flexDirection: "column" as const, alignItems: isMe ? "flex-end" : "flex-start" }}>
                    <div style={{ fontSize: "0.7rem", color: "#6b5f54", marginBottom: "2px" }}>{msg.sender_name}</div>
                    <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: isMe ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: isMe ? "#5a3e2b" : "#f0ece6", color: isMe ? "#fff" : "#5a3e2b", fontSize: "0.88rem", lineHeight: 1.5 }}>{msg.message}</div>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              <button onClick={sendMessage} style={{ padding: "8px 16px", background: "#5a3e2b", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.85rem", cursor: "pointer" }}>Send</button>
            </div>
          </div>
        )}
        ...
        {activeTab === "receipts" && (
          <div style={{ background: "#fff", borderRadius: "12px", padding: "1.5rem", border: "1px solid #e8e2da" }}>
            <div style={{ fontSize: "0.9rem", color: "#5a3e2b", marginBottom: "1rem" }}>Receipts & Invoices</div>
            <p style={{ fontSize: "0.8rem", color: "#6b5f54", marginBottom: "1rem" }}>{plannerEvent?.brand_name || "The brand"} can approve or reject these from their portal.</p>

            <div style={{ background: "#faf8f5", borderRadius: "10px", padding: "1rem", border: "1px solid #f0ece6", marginBottom: "1.5rem" }}>
              <div style={{ fontSize: "0.82rem", color: "#5a3e2b", marginBottom: "10px", fontWeight: 500 }}>Upload new receipt or invoice</div>
              <input placeholder="Description e.g. Venue deposit" value={newReceipt.description} onChange={e => setNewReceipt({...newReceipt, description: e.target.value})} style={{ width: "100%", padding: "8px 10px", border: "1px solid #e8e2da", borderRadius: "8px", fontSize: "0.85rem", fontFamily: "Georgia, serif", marginBottom: "8px", boxSizing: "border-box" as const }} />
              <input placeholder="Amount $" value={newReceipt.amount} onChange={e => setNewReceipt({...newReceipt, amount: e.target.value})} style={{ width: "100%", padding: "8px 10px", border: "1px solid #e8e2da", borderRadius: "8px", fontSize: "0.85rem", fontFamily: "Georgia, serif", marginBottom: "8px", boxSizing: "border-box" as const }} />
              <div onClick={() => document.getElementById("receipt-upload")?.click()} style={{ border: "2px dashed #e8e2da", borderRadius: "8px", padding: "12px", textAlign: "center" as const, cursor: "pointer", marginBottom: "8px", background: selectedReceiptFile ? "#f0faf0" : "#fff" }}>
                ...
                {selectedReceiptFile ? <div style={{ fontSize: "0.82rem", color: "#4a7c59" }}>✓ {selectedReceiptFile.name}</div> : <div style={{ fontSize: "0.82rem", color: "#6b5f54" }}>Click to select file (PDF or image)</div>}
              </div>
              <button onClick={uploadReceipt} disabled={uploading || !newReceipt.description.trim() || !selectedReceiptFile} style={{ padding: "8px 16px", background: selectedReceiptFile && newReceipt.description ? "#5a3e2b" : "#d4c5b0", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.85rem", cursor: selectedReceiptFile && newReceipt.description ? "pointer" : "not-allowed", fontFamily: "Georgia, serif" }}>
                {uploading ? "Uploading..." : "Upload"}
              </button>
            </div>

            {receipts.length === 0 ? (
              <p style={{ fontSize: "0.85rem", color: "#6b5f54" }}>No receipts uploaded yet.</p>
            ) : (
              receipts.map(receipt => (
                <div key={receipt.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", borderRadius: "10px", background: "#faf8f5", marginBottom: "8px", border: "1px solid #f0ece6" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "0.88rem", color: "#5a3e2b" }}>{receipt.description}</div>
                    <div style={{ fontSize: "0.75rem", color: "#6b5f54", marginTop: "2px" }}>{receipt.file_name} · {new Date(receipt.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</div>
                  </div>
                  <div style={{ fontSize: "0.95rem", color: "#c4956a", fontWeight: 500 }}>${Number(receipt.amount).toFixed(2)}</div>
                  <span style={{ fontSize: "0.7rem", padding: "2px 8px", borderRadius: "20px", background: receipt.status === "approved" ? "#4a7c5922" : receipt.status === "rejected" ? "#c0392b22" : "#f0ece6", color: receipt.status === "approved" ? "#4a7c59" : receipt.status === "rejected" ? "#c0392b" : "#6b5f54" }}>
                    {receipt.status === "approved" ? "Approved" : receipt.status === "rejected" ? "Rejected" : "Pending"}
                  </span>
                  <a href={receipt.file_url} download target="_blank" rel="noopener noreferrer" style={{ fontSize: "11px", padding: "3px 8px", background: "#5a3e2b", color: "#fff", borderRadius: "6px", textDecoration: "none" }}>↓</a>
