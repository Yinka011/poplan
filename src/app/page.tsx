...
export default function HomePage() {
  ...
  const inp = {
    width: "100%", padding: "0.75rem", border: "1px solid #e8e0d5", borderRadius: "8px",
  ...
  const btn = (bg: string, color: string = "#fff") => ({
    ...
    border: bg === "#fff" ? "1px solid #2c1810" : "none", borderRadius: "8px",
        ...
        <div style={{ background: "#fff", borderRadius: "16px", padding: "2rem", border: "1px solid #e8e0d5" }}>
          ...
          {mode === "home" && (
            <div>
              ...
              <button onClick={() => { setMode("organizer-login"); setError(""); setEmail(""); setPassword(""); }} style={btn("#2c1810")}>I am an Organizer</button>
              <button onClick={() => { setMode("brand-login"); setError(""); setEmail(""); setPassword(""); }} style={btn("#fff", "#2c1810")}>I am a Brand</button>
            </div>
          )}
          ...
          {mode === "organizer-login" && (
            <form onSubmit={handleOrganizerLogin}>
              ...
              <button type="button" onClick={() => { setMode("forgot"); setError(""); }} style={{ width: "100%", padding: "0.5rem", background: "transparent", color: "#8b7355", border: "none", fontSize: "0.85rem", fontFamily: "Georgia, serif", cursor: "pointer", marginBottom: "6px" }}>Forgot password?</button>
              <button type="button" onClick={() => setMode("home")} style={{ width: "100%", padding: "0.5rem", background: "transparent", color: "#8b7355", border: "none", fontSize: "0.85rem", fontFamily: "Georgia, serif", cursor: "pointer" }}>Back</button>
            </form>
          )}
          ...
          {mode === "brand-login" && (
            <form onSubmit={handleBrandLogin}>
              ...
              <p style={{ fontSize: "0.85rem", color: "#8b7355", textAlign: "center", marginBottom: "1.5rem" }}>Sign in to access your portal</p>
              <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} required style={inp} />
              <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} required style={inp} />
              {error && <p style={{ color: "#c0392b", fontSize: "0.85rem", marginBottom: "1rem", textAlign: "center" }}>{error}</p>}
              <button type="submit" disabled={loading} style={btn("#2c1810")}>{loading ? "Signing in..." : "Sign in"}</button>
          ...
          {mode === "forgot" && (
            sent ? (
              <div style={{ textAlign: "center" }}>
                ...
                <button onClick={() => { setMode("home"); setSent(false); setEmail(""); }} style={btn("#2c1810")}>Back to sign in</button>
              </div>
            ) : (
              <form onSubmit={handleForgotPassword}>
                ...
                {error && <p style={{ color: "#c0392b", fontSize: "0.85rem", marginBottom: "1rem", textAlign: "center" }}>{error}</p>}
                <button type="submit" disabled={loading} style={btn("#2c1810")}>{loading ? "Sending..." : "Send reset link"}</button>
                <button type="button" onClick={() => setMode("home")} style={{ width: "100%", padding: "0.5rem", background: "transparent", color: "#8b7355", border: "none", fontSize: "0.85rem", fontFamily: "Georgia, serif", cursor: "pointer" }}>Back</button>
              </form>
            )
          )}
