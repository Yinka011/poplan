...
export default function LoginPage() {
  ...
  const handleOrganizer = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError("");
  ...
  const handleBrand = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithPassword({ email, password });
    if (error) { setError("Invalid email or password"); setLoading(false); }
    else { window.location.href = "/login/brand"; }
  };
        ...
        <div style={{ background: "#fff", borderRadius: "16px", padding: "2rem", border: "1px solid #e8e0d5" }}>
          ...
          <form>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <button type="submit" onClick={handleOrganizer} disabled={loading} style={{ width: "100%", padding: "0.85rem", background: "#2c1810", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.95rem", fontFamily: "Georgia, serif", cursor: "pointer" }}>
                {loading ? "Signing in..." : "Sign in as Organizer"}
              </button>
              <button type="submit" onClick={handleBrand} disabled={loading} style={{ width: "100%", padding: "0.85rem", background: "#fff", color: "#2c1810", border: "1px solid #2c1810", borderRadius: "8px", fontSize: "0.95rem", fontFamily: "Georgia, serif", cursor: "pointer" }}>
