  ...
  return (
    <div style={{ background: "#fff", borderRadius: "16px", padding: "1.5rem", border: "1px solid #e8e2da", marginBottom: "1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "1.25rem" }}>
        <div style={{ fontSize: "1rem", color: "#5a3e2b", fontFamily: "Georgia, serif" }}>Announcements</div>
        {visible.length > 0 && (
          <span style={{ fontSize: "0.7rem", padding: "2px 8px", borderRadius: "20px", background: "#f0ece6", color: "#6b5f54" }}>{visible.length}</span>
        )}
      </div>

      {visible.length === 0 ? (
        <p style={{ fontSize: "0.85rem", color: "#6b5f54" }}>No new announcements.</p>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {visible.map(a => (
            <div key={a.id} style={{ background: a.pinned ? "#fdf8f3" : "#faf8f5", borderRadius: "10px", padding: "1rem", border: `1px solid ${a.pinned ? "#e8e2da" : "#f0ece6"}`, position: "relative" as const }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  ...
                  <span style={{ fontSize: "0.75rem", color: "#c4956a", fontWeight: 500 }}>{a.author}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span style={{ fontSize: "0.75rem", color: "#6b5f54" }}>{formatDate(a.created_at)}</span>
                  {!a.pinned && (
                    <button
