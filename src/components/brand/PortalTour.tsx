...
export default function PortalTour({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "1rem", pointerEvents: "none" }}>
      <div style={{ background: "#fff", borderRadius: "20px", padding: "2rem 2rem 1.5rem", maxWidth: "460px", width: "100%", boxShadow: "0 8px 40px #00000022", border: "1px solid #e8e2da", fontFamily: "Georgia, serif", pointerEvents: "all", marginTop: "1rem" }}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          <div style={{ fontSize: "2rem", color: "#f0ece6", lineHeight: 1, marginBottom: "0.5rem" }}>{current.number}</div>
          <div style={{ fontSize: "1.05rem", color: "#5a3e2b", marginBottom: "0.5rem" }}>{current.title}</div>
          <p style={{ fontSize: "0.85rem", color: "#6b5f54", lineHeight: 1.75, margin: 0 }}>{current.description}</p>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "0.85rem", color: "#6b5f54", background: "transparent", border: "1px solid #e8e2da", borderRadius: "8px", padding: "8px 14px", cursor: "pointer", fontFamily: "Georgia, serif", visibility: step === 0 ? "hidden" : "visible" }}
          >
            ← Back
          </button>
          <button
            onClick={() => isLast ? onClose() : setStep(s => s + 1)}
            style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 20px", background: "#5a3e2b", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.85rem", cursor: "pointer", fontFamily: "Georgia, serif" }}
          >
            {isLast ? "Done ✓" : "Next →"}
          </button>
        </div>
