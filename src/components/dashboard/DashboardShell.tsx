    ...
    <div style={{ minHeight: "100vh", background: "#f5f0ea", display: "flex", fontFamily: "Georgia, serif" }}>
      <div style={{ width: "220px", minHeight: "100vh", background: "#2c1810", display: "flex", flexDirection: "column" as const, flexShrink: 0, position: "fixed" as const, left: 0, top: 0, bottom: 0, zIndex: 10 }}>
        ...
        <nav style={{ flex: 1, padding: "1rem 0" }}>
          {navItems.map(item => (
            <a key={item.label} href={item.href} style={{ display: "block", padding: "10px 1.25rem", fontSize: "0.85rem", color: pathname === item.href ? "#fff" : "#c8b89a", background: pathname === item.href ? "#3d2415" : "transparent", textDecoration: "none", borderLeft: pathname === item.href ? "2px solid #b87333" : "2px solid transparent" }}>
              {item.label}
            </a>
          ))}
        </nav>
        <div style={{ padding: "1rem 1.25rem", borderTop: "1px solid #3d2415" }}>
          ...
          <Link href="/login/organizer/events" style={{ fontSize: "0.8rem", color: "#c8b89a", textDecoration: "none", display: "block", marginBottom: "6px" }}>All events</Link>
          <Link href="/" style={{ fontSize: "0.8rem", color: "#c8b89a", textDecoration: "none" }}>Sign out</Link>
        </div>
      </div>
