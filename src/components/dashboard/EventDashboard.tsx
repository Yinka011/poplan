...
export function EventDashboard({ event }: EventDashboardProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div style={{ background: "#1B3A2D", borderRadius: "16px", padding: "1.75rem 2rem", color: "#fff" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "1.5rem", alignItems: "start" }}>

          {/* Spots to fill */}
          <div>
            <div style={{ fontSize: "0.65rem", color: "#d4c87a", letterSpacing: "0.1em", marginBottom: "6px" }}>SPOTS TO FILL</div>
            {editingSpots ? (
              <div style={{ display: "flex", flexDirection: "column" as const, gap: "6px" }}>
                <input type="number" value={newSpots} onChange={e => setNewSpots(e.target.value)} style={{ width: "60px", padding: "4px 6px", border: "1px solid #E8C97A", borderRadius: "4px", fontSize: "1.2rem", fontFamily: "Georgia, serif", background: "#2a4d3e", color: "#fff" }} autoFocus />
                <div style={{ display: "flex", gap: "4px" }}>
                  <button onClick={saveSpots} style={{ padding: "3px 8px", background: "#E8C97A", color: "#fff", border: "none", borderRadius: "4px", fontSize: "11px", cursor: "pointer" }}>Save</button>
                  <button onClick={() => setEditingSpots(false)} style={{ padding: "3px 8px", background: "transparent", border: "1px solid #d4c87a44", color: "#d4c87a", borderRadius: "4px", fontSize: "11px", cursor: "pointer" }}>Cancel</button>
                </div>
              </div>
            ) : (
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <div style={{ fontSize: "2rem", fontFamily: "Georgia, serif", fontWeight: "normal" }}>{spotsToFill}</div>
                <button onClick={() => setEditingSpots(true)} title="Edit" style={{ background: "transparent", border: "none", cursor: "pointer", color: "#d4c87a", padding: "2px", opacity: 0.7 }} onMouseEnter={e => (e.currentTarget.style.opacity = "1")} onMouseLeave={e => (e.currentTarget.style.opacity = "0.7")}>
                  <PencilIcon />
                </button>
              </div>
            )}
            <div style={{ fontSize: "0.72rem", color: "#d4c87a", marginTop: "4px" }}>{brandsCount} confirmed</div>
          </div>

          {/* Brands confirmed */}
          <div>
            <div style={{ fontSize: "0.65rem", color: "#d4c87a", letterSpacing: "0.1em", marginBottom: "6px" }}>BRANDS CONFIRMED</div>
            <div style={{ fontSize: "2rem", fontFamily: "Georgia, serif", fontWeight: "normal" }}>{brandsCount}</div>
            <div style={{ fontSize: "0.72rem", color: "#d4c87a", marginTop: "4px" }}>of {spotsToFill} spots</div>
          </div>

          {/* Outstanding tasks */}
          <div>
            <div style={{ fontSize: "0.65rem", color: "#d4c87a", letterSpacing: "0.1em", marginBottom: "6px" }}>OUTSTANDING TASKS</div>
            ...
            <div style={{ fontSize: "0.72rem", color: "#d4c87a", marginTop: "4px" }}>tasks remaining</div>
          </div>
      ...
      {/* Venue address */}
      <div style={{ background: "#fff", borderRadius: "12px", padding: "1.25rem", border: "1px solid #e4ebe6", position: "relative" as const }}>
        <div style={{ fontSize: "0.75rem", color: "#4a5a52", letterSpacing: "0.05em", marginBottom: "0.5rem" }}>VENUE ADDRESS</div>
        {editingAddress ? (
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            <input type="text" value={newAddress} onChange={e => setNewAddress(e.target.value)} style={{ flex: 1, padding: "6px 10px", border: "1px solid #E8C97A", borderRadius: "6px", fontSize: "0.9rem", fontFamily: "Georgia, serif" }} autoFocus />
            <button onClick={saveAddress} style={{ padding: "6px 12px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "6px", fontSize: "12px", cursor: "pointer" }}>Save</button>
            <button onClick={() => setEditingAddress(false)} style={{ padding: "6px 12px", background: "transparent", border: "1px solid #e4ebe6", borderRadius: "6px", fontSize: "12px", cursor: "pointer" }}>Cancel</button>
          </div>
        ) : (
          <>
            <div style={{ fontSize: "0.95rem", color: "#1B3A2D", fontFamily: "Georgia, serif" }}>{venueAddress || "No address set yet"}</div>
            <button onClick={() => setEditingAddress(true)} title="Edit address" style={{ position: "absolute" as const, top: "10px", right: "10px", background: "transparent", border: "none", cursor: "pointer", color: "#E8C97A" }}>
              <PencilIcon />
            </button>
          </>
        )}
