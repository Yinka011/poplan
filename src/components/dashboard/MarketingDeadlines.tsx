...
export default function MarketingPlans({ event }: Props) {
  ...
  if (loading) return <div style={{ fontSize: "0.85rem", color: "#4a5a52" }}>Loading...</div>;

  return (
    <div style={{ background: "#fff", borderRadius: "16px", padding: "1.5rem", border: "1px solid #e4ebe6", fontFamily: "Georgia, serif" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        <h2 style={{ fontSize: "1rem", color: "#1B3A2D", fontWeight: "normal", margin: 0 }}>Marketing Plans</h2>
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          <span style={{ fontSize: "0.78rem", color: "#4a5a52" }}>{completedItems}/{totalItems} done</span>
        ...
        <button onClick={() => setAdding(!adding)} style={{ padding: "5px 12px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.78rem", cursor: "pointer", fontFamily: "Georgia, serif" }}>+ Add task</button>
      </div>

      {totalItems > 0 && (
        <div style={{ height: "3px", background: "#f0f4f1", borderRadius: "2px", marginBottom: "1rem" }}>
          <div style={{ height: "100%", width: `${totalItems > 0 ? (completedItems / totalItems) * 100 : 0}%`, background: "#4a7c59", borderRadius: "2px" }} />
        </div>
      )}

      {adding && (
        <div style={{ background: "#fff", borderRadius: "12px", padding: "1rem", marginBottom: "1rem", border: "1px solid #e4ebe6" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={addItem} style={{ padding: "6px 14px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.78rem", cursor: "pointer" }}>Save</button>
            <button onClick={() => setAdding(false)} style={{ padding: "6px 14px", background: "transparent", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.78rem", cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      )}

      {Object.keys(grouped).length === 0 ? (
        <div style={{ textAlign: "center", padding: "1.5rem", color: "#4a5a52", fontSize: "0.82rem" }}>No marketing tasks yet.</div>
      ) : (
        Object.entries(grouped).map(([channel, channelItems]) => {
          ...
          return (
            <div key={channel} style={{ marginBottom: "0.75rem", background: "#fff", borderRadius: "10px", border: "1px solid #e4ebe6", overflow: "hidden" }}>
              <div onClick={() => setCollapsed(prev => ({...prev, [channel]: !prev[channel]}))} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 14px", cursor: "pointer", background: "#f8faf8" }}>
                <span style={{ fontSize: "0.82rem", color: "#1B3A2D", fontWeight: 500, flex: 1 }}>{channel}</span>
                <span style={{ fontSize: "0.7rem", color: "#4a5a52" }}>{done}/{channelItems.length}</span>
                <span style={{ fontSize: "0.7rem", color: "#4a5a52" }}>{isCollapsed ? "▸" : "▾"}</span>
              </div>
              {!isCollapsed && channelItems.map(item => (
                <div key={item.id} style={{ display: "flex", alignItems: "flex-start", gap: "10px", padding: "9px 14px", borderTop: "1px solid #f0f4f1", background: isOverdue(item) ? "#fff8f8" : "#fff" }}>
                  ...
                  {editing === item.id ? (
                    <div style={{ flex: 1, display: "flex", gap: "6px", flexWrap: "wrap" as const }}>
                      <input value={editData.task} onChange={e => setEditData({...editData, task: e.target.value})} style={{ flex: 1, padding: "5px 8px", border: "1px solid #e4ebe6", borderRadius: "6px", fontSize: "0.78rem", fontFamily: "Georgia, serif", minWidth: "150px" }} />
                      <select value={editData.channel} onChange={e => setEditData({...editData, channel: e.target.value})} style={{ padding: "5px 8px", border: "1px solid #e4ebe6", borderRadius: "6px", fontSize: "0.78rem", fontFamily: "Georgia, serif" }}>
                        {PLATFORMS.map(p => <option key={p}>{p}</option>)}
                      </select>
                      <input type="date" value={editData.due_date} onChange={e => setEditData({...editData, due_date: e.target.value})} style={{ padding: "5px 8px", border: "1px solid #e4ebe6", borderRadius: "6px", fontSize: "0.78rem" }} />
                      <input placeholder="Assigned to" value={editData.assigned_to} onChange={e => setEditData({...editData, assigned_to: e.target.value})} style={{ padding: "5px 8px", border: "1px solid #e4ebe6", borderRadius: "6px", fontSize: "0.78rem", fontFamily: "Georgia, serif", width: "110px" }} />
                      <button onClick={() => saveEdit(item.id)} style={{ padding: "4px 10px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "6px", fontSize: "0.72rem", cursor: "pointer" }}>Save</button>
                      <button onClick={() => setEditing(null)} style={{ padding: "4px 8px", background: "transparent", border: "1px solid #e4ebe6", borderRadius: "6px", fontSize: "0.72rem", cursor: "pointer" }}>✕</button>
                    </div>
                  ) : (
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: "0.85rem", color: item.completed ? "#b0a090" : "#1B3A2D", textDecoration: item.completed ? "line-through" : "none" }}>{item.task}</div>
                      <div style={{ display: "flex", gap: "8px", marginTop: "2px", flexWrap: "wrap" as const }}>
                        <span style={{ fontSize: "0.7rem", color: isOverdue(item) ? "#c0392b" : "#4a5a52" }}>{isOverdue(item) ? "⚠ " : ""}Due {formatDate(item.due_date)}</span>
                        {item.assigned_to && <span style={{ fontSize: "0.7rem", color: "#E8C97A" }}>→ {item.assigned_to}</span>}
