...
export default function PaymentTracker() {
  ...
  const iconBtn = (onClick: () => void, icon: React.ReactNode, title: string, danger = false) => (
    <button
      onClick={onClick}
      title={title}
      style={{ background: "transparent", border: "none", cursor: "pointer", padding: "4px 5px", borderRadius: "5px", color: danger ? "#E8C97A" : "#E8C97A", display: "flex", alignItems: "center" }}
  ...
  return (
    <div style={{ background: "#fff", borderRadius: "16px", padding: "1.5rem", border: "1px solid #e4ebe6" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", marginBottom: "1.25rem" }}>
        <div style={{ background: "#f8faf8", borderRadius: "10px", padding: "0.75rem 1rem", border: "1px solid #f0f4f1" }}>
          <div style={{ fontSize: "0.65rem", color: "#4a5a52", letterSpacing: "0.08em", marginBottom: "2px" }}>TOTAL FEES</div>
          <div style={{ fontSize: "1.1rem", color: "#1B3A2D", fontFamily: "Georgia, serif" }}>${totalFees.toFixed(2)}</div>
        </div>
        <div style={{ background: "#f8faf8", borderRadius: "10px", padding: "0.75rem 1rem", border: "1px solid #f0f4f1" }}>
          <div style={{ fontSize: "0.65rem", color: "#4a5a52", letterSpacing: "0.08em", marginBottom: "2px" }}>RECEIVED</div>
      ...
      {adding && (
        <div style={{ background: "#f8faf8", borderRadius: "12px", padding: "1rem", marginBottom: "1rem", border: "1px solid #e4ebe6" }}>
          <div style={{ fontSize: "0.85rem", color: "#1B3A2D", marginBottom: "8px", fontFamily: "Georgia, serif" }}>Add new brand</div>
          <input placeholder="Brand name" value={newBrand.name} onChange={e => setNewBrand({...newBrand, name: e.target.value})} style={{ width: "100%", padding: "8px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.85rem", fontFamily: "Georgia, serif", marginBottom: "8px", boxSizing: "border-box" as const }} />
          <input placeholder="Brand email" value={newBrand.email} onChange={e => setNewBrand({...newBrand, email: e.target.value})} style={{ width: "100%", padding: "8px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.85rem", fontFamily: "Georgia, serif", marginBottom: "8px", boxSizing: "border-box" as const }} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "8px" }}>
            <input placeholder="Fee owed" value={newBrand.fee_owed} onChange={e => setNewBrand({...newBrand, fee_owed: e.target.value})} style={{ padding: "8px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.85rem", fontFamily: "Georgia, serif" }} />
            <input placeholder="Amount paid" value={newBrand.amount_paid} onChange={e => setNewBrand({...newBrand, amount_paid: e.target.value})} style={{ padding: "8px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.85rem", fontFamily: "Georgia, serif" }} />
          </div>
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={addBrand} style={{ padding: "7px 16px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.85rem", cursor: "pointer", fontFamily: "Georgia, serif" }}>Save</button>
        ...
        <thead>
          <tr style={{ borderBottom: "1px solid #e4ebe6" }}>
            {["Brand", "Fee Owed", "Amount Paid", "Balance", "Status", "Portal", ""].map((h, i) => (
              <th key={i} style={{ textAlign: "left", padding: "8px 10px", fontSize: "11px", color: "#4a5a52", fontWeight: 400 }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {brands.map((brand) => (
            <tr key={brand.id} style={{ borderBottom: "1px solid #f0f4f1" }}>
              <td style={{ padding: "10px", fontWeight: 500, fontFamily: "Georgia, serif" }}>
                <a href={`/login/organizer/events/atlanta/brands/${encodeURIComponent(brand.name)}`} style={{ color: "#E8C97A", textDecoration: "none", borderBottom: "1px solid #E8C97A44" }} onMouseEnter={e => (e.currentTarget.style.color = "#1B3A2D")} onMouseLeave={e => (e.currentTarget.style.color = "#E8C97A")}>
                  {brand.name}
                </a>
              </td>
              <td style={{ padding: "10px", color: "#1B3A2D" }}>
                {editing === brand.id ? (
                  <input type="number" value={newFee} onChange={e => setNewFee(e.target.value)} style={{ width: "70px", padding: "4px", border: "1px solid #E8C97A", borderRadius: "4px", fontSize: "13px" }} autoFocus />
                ) : (
              ...
              <td style={{ padding: "10px" }}>
                {editing === brand.id ? (
                  <input type="number" value={newAmount} onChange={e => setNewAmount(e.target.value)} style={{ width: "70px", padding: "4px", border: "1px solid #E8C97A", borderRadius: "4px", fontSize: "13px" }} />
                ) : (
              ...
              <td style={{ padding: "10px" }}>
                {brand.email ? (
                  brand.invited ? (
                    ...
                    <button onClick={() => inviteBrand(brand)} disabled={inviting === brand.id} style={{ fontSize: "11px", padding: "3px 10px", background: "#E8C97A22", color: "#E8C97A", border: "1px solid #E8C97A44", borderRadius: "20px", cursor: "pointer" }}>
                      {inviting === brand.id ? "..." : "✉"}
                    </button>
                  )
                ) : (
              ...
              <td style={{ padding: "10px" }}>
                {editing === brand.id ? (
                  <div style={{ display: "flex", gap: "4px" }}>
                    <button onClick={() => updatePayment(brand)} style={{ fontSize: "11px", padding: "4px 10px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer" }}>Save</button>
                    <button onClick={() => setEditing(null)} style={{ fontSize: "11px", padding: "4px 10px", background: "transparent", border: "1px solid #e4ebe6", borderRadius: "4px", cursor: "pointer" }}>Cancel</button>
                  </div>
                ) : (
