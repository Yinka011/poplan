...
export default function MoodBoard({ eventSlug, userEmail, userName }: Props) {
  ...
  if (loading) return <div style={{ padding: "2rem", textAlign: "center", color: "#4a5a52", fontSize: "0.85rem" }}>Loading mood board...</div>;
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        <div>
          <div style={{ fontSize: "0.6rem", color: "#4a5a52", letterSpacing: "0.2em", marginBottom: "4px" }}>SHARED INSPIRATION</div>
          <div style={{ fontSize: "1rem", color: "#1B3A2D" }}>{pins.length} pin{pins.length !== 1 ? "s" : ""}</div>
        </div>
        <button onClick={() => setAdding(!adding)} style={{ padding: "8px 18px", background: "#1B3A2D", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.82rem", cursor: "pointer", fontFamily: "Georgia, serif" }}>+ Add pin</button>
      </div>

      {/* Add pin form */}
      {adding && (
        <div style={{ background: "#fff", borderRadius: "16px", padding: "1.5rem", marginBottom: "1.5rem", border: "1px solid #e4ebe6" }}>
          <div style={{ fontSize: "0.82rem", color: "#1B3A2D", marginBottom: "1rem", fontWeight: 500 }}>Add to mood board</div>
          ...
          <div style={{ marginBottom: "1rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
              <div style={{ flex: 1, height: "1px", background: "#e4ebe6" }} />
              <span style={{ fontSize: "0.72rem", color: "#4a5a52" }}>or paste a URL</span>
              <div style={{ flex: 1, height: "1px", background: "#e4ebe6" }} />
            </div>
            <input placeholder="https://..." value={newPin.image_url} onChange={e => { setNewPin({...newPin, image_url: e.target.value}); if (e.target.value) { setSelectedFile(null); setPreview(null); } }} style={{ width: "100%", padding: "8px 10px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.82rem", fontFamily: "Georgia, serif", boxSizing: "border-box" as const }} />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "8px", marginBottom: "8px" }}>
            <input placeholder="Label e.g. Backdrop inspo" value={newPin.label} onChange={e => setNewPin({...newPin, label: e.target.value})} style={{ padding: "8px 10px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.82rem", fontFamily: "Georgia, serif" }} />
            <select value={newPin.category} onChange={e => setNewPin({...newPin, category: e.target.value})} style={{ padding: "8px 10px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.82rem", fontFamily: "Georgia, serif" }}>
              {ADD_CATEGORIES.map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
          <input placeholder="Notes (optional)" value={newPin.notes} onChange={e => setNewPin({...newPin, notes: e.target.value})} style={{ width: "100%", padding: "8px 10px", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.82rem", fontFamily: "Georgia, serif", marginBottom: "8px", boxSizing: "border-box" as const }} />

          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={addPin} disabled={uploading || (!selectedFile && !newPin.image_url) || !newPin.label.trim()} style={{ padding: "8px 18px", background: (!selectedFile && !newPin.image_url) || !newPin.label.trim() ? "#d4c5b0" : "#1B3A2D", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.82rem", cursor: "pointer", fontFamily: "Georgia, serif" }}>
              {uploading ? "Uploading..." : "Add to board"}
            </button>
            <button onClick={() => { setAdding(false); setSelectedFile(null); setPreview(null); setNewPin({ label: "", category: "Decor", notes: "", image_url: "" }); }} style={{ padding: "8px 16px", background: "transparent", border: "1px solid #e4ebe6", borderRadius: "8px", fontSize: "0.82rem", cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: "6px", marginBottom: "1.5rem", flexWrap: "wrap" as const }}>
        {CATEGORIES.map(cat => (
          <button key={cat} onClick={() => setFilter(cat)} style={{ padding: "5px 14px", background: filter === cat ? "#1B3A2D" : "#fff", color: filter === cat ? "#fff" : "#4a5a52", border: "1px solid " + (filter === cat ? "#1B3A2D" : "#e4ebe6"), borderRadius: "20px", fontSize: "0.75rem", cursor: "pointer", fontFamily: "Georgia, serif" }}>{cat}</button>
        ))}
      </div>

      {/* Pins grid */}
      {filteredPins.length === 0 ? (
        <div style={{ textAlign: "center", padding: "4rem", color: "#4a5a52", fontSize: "0.85rem" }}>
        ...
        <div style={{ columns: "3", columnGap: "1rem" }}>
          {filteredPins.map(pin => {
            ...
            return (
              <div key={pin.id} style={{ breakInside: "avoid", marginBottom: "1rem", background: "#fff", borderRadius: "14px", overflow: "hidden", border: "1px solid #e4ebe6" }}>
                <div style={{ position: "relative" as const }}>
                  ...
                  <div style={{ position: "absolute", top: "8px", right: "8px", display: "flex", gap: "4px" }}>
                    <span style={{ fontSize: "0.65rem", padding: "2px 8px", borderRadius: "20px", background: "rgba(255,255,255,0.9)", color: "#4a5a52" }}>{pin.category}</span>
                  </div>
                </div>
                <div style={{ padding: "0.75rem 1rem" }}>
                  <div style={{ fontSize: "0.88rem", color: "#1B3A2D", fontWeight: 500, marginBottom: "2px" }}>{pin.label}</div>
                  {pin.notes && <div style={{ fontSize: "0.75rem", color: "#4a5a52", marginBottom: "6px", lineHeight: 1.4 }}>{pin.notes}</div>}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <button onClick={() => setActivePin(isActive ? null : pin.id)} style={{ background: "transparent", border: "none", cursor: "pointer", color: pinComments.length > 0 ? "#E8C97A" : "#b0a090", fontSize: "0.78rem", display: "flex", alignItems: "center", gap: "3px", padding: 0 }}>
                  ...
                  {isActive && (
                    <div style={{ marginTop: "10px", borderTop: "1px solid #f0f4f1", paddingTop: "10px" }}>
                      {pinComments.map(c => (
                        <div key={c.id} style={{ marginBottom: "6px", padding: "6px 8px", background: "#f8faf8", borderRadius: "6px", borderLeft: c.sender_email === userEmail ? "2px solid #E8C97A" : "2px solid #e4ebe6" }}>
                          <div style={{ fontSize: "0.65rem", color: "#4a5a52", marginBottom: "2px" }}>{c.sender_name}</div>
                          <div style={{ fontSize: "0.78rem", color: "#1B3A2D" }}>{c.message}</div>
                        </div>
                      ))}
